...
class SetGenerator(nn.Module):
    def __init__(self, cfg):
        ...
        self.n_probs = torch.Tensor(cfg.n_distribution)
        ...
        if self.learn_from_latent:
            self.mlp1 = MLP(self.latent_channels, 1, cfg.hidden_set_gen, cfg.num_mlp_layers)
        if self.predict_molecular_formula:
            self.mlp2 = MLP(self.latent_channels, cfg.num_atom_types, cfg.hidden_set_gen, cfg.num_mlp_layers)
...
class RandomSetGenerator(SetGenerator):
    def forward(self, latent: Tensor, n: int = None):
        ...
        n, predicted_n, predicted_formula = super().forward(latent, n)
...
class MLPGenerator(SetGenerator):
    def __init__(self, cfg):
        ...
        self.mlp = MLP(self.latent_channels, self.max_num_nodes * self.set_channels, self.mlp_gen_hidden, nb_layers=2)

    def forward(self, latent: Tensor, n: int = None):
        batch_size = latent.shape[0]
        n, predicted_n, predicted_formula = super().forward(latent, n)

        points = self.mlp(latent).reshape(batch_size, self.max_num_nodes, self.set_channels)
        points = points[:, :n, :]
        return points, predicted_n, predicted_formula


class TopKSetGenerator(SetGenerator):
    def __init__(self, cfg):
        super().__init__(cfg)
        self.set_channels = cfg.set_channels
